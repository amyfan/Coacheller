  ...
  public void onModuleLoad() {
    ...
    class QueryHandler implements ClickHandler, KeyUpHandler {
      ...
      private void getSetsFromServer() {
        ...
        coachellerService.getSets(year, day, new AsyncCallback<List<Set>>() {
          ...
          public void onSuccess(List<Set> result) {
            ...
            String htmlString = "";
            for (Set set : result) {
              htmlString += set.getArtistName() + ", ";
            }
            serverResponseLabel.setHTML(htmlString);
            dialogBox.center();
            closeButton.setFocus(true);
          }
        });
      ...
      private void getRatingsFromServer() {
        ...
        textToServerLabel.setText(email);
        serverResponseLabel.setText("");
        coachellerService.getRatingsBySetArtist(email, artist,
            new AsyncCallback<List<RatingGwt>>() {
              ...
              public void onSuccess(List<RatingGwt> result) {
                ...
                // JSONUtils.convertJSONArrayStringToSetString(result);
                String htmlString = "";
                for (RatingGwt rating : result) {
                  htmlString += "setId:" + rating.getSetId() + ",raterId:" + rating.getRaterId()
